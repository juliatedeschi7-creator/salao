    ...
    try {
      ...
      if (isCliente && salaoSlug) {
        ...
        await supabase.from('profiles').update({
          salao_id: salao.id, aprovado: true,
        }).eq('id', data.user.id)
    ...
    <div className="min-h-screen bg-white flex flex-col">
      <div className="bg-gray-900 px-6 pt-16 pb-12 flex flex-col items-center">
        ...
        <h1 className="text-white text-2xl font-bold">
          {isCliente ? 'Criar sua conta' : 'Cadastrar Salão'}
        </h1>
        <p className="text-gray-400 text-sm mt-1">
          {isCliente ? 'Acesse os serviços do salão' : 'Comece a organizar seu espaço'}
        </p>
      </div>
